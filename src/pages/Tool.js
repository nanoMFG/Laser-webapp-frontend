...
import SearchByLaser from "../components/GrresqQueryBox/SearchByLaser";
...
const Tool = () => {
  ...
  const [mouseOverLasers, setMouseOverLasers] = useState(false)
  ...
  useEffect(() => {
    document.getElementById('laser-btn').innerHTML = showLasers ? '&#8211;' : '+'
  }, [showLasers])
  useEffect(() => {
      ...
      <div className='w-full md:flex flex-row mt-5 '>
        <div className='md:w-1/2 px-10'>
          ...
          <div className='h-screen-3/4 overflow-y-scroll border p-3'>
            ...
            <section className='w-full flex flex-col mb-5'>
              <div className='flex justify-center align-middle mb-4'>
                <h2 className='text-center text-3xl font-bold mr-2'>Laser</h2>
                <div className='flex flex-col justify-center'>
                  <img className='w-6 h-6 mr-2' src="https://img.icons8.com/material-two-tone/24/000000/help.png"
                       alt='?'
                       onMouseOver={() => setMouseOverLasers(true)}
                       onMouseLeave={() => setMouseOverLasers(false)}/>
                </div>
                <button
                  className='w-9 h-9 self-center text-center bg-gray-400 hover:bg-blue-700 text-white text-3xl font-bold rounded focus:outline-none focus:shadow-outline'
                  type='button' id='laser-btn' onClick={() => setShowLasers(!showLasers)}
                >
                  +
                </button>
              </div>
              {mouseOverLasers && <div className='mx-auto my-2 p-2 bg-gray-200'>
                Static parameters of the laser such as tube diameter
              </div>}
              {showLasers || <hr/>}
              {showLasers && <SearchByLaser/>}
            </section>
            <section className='w-full flex flex-col mb-5'>
              <div className='flex justify-center align-middle mb-4'>
